...
public class ControlTutorialManager : MonoBehaviour
{
    ...
    private void StepDialog()
    {
        ...
        seq.AppendCallback(() => { dialogs[currentDialogIndex - 1].gameObject.SetActive(false); });
        seq.AppendCallback(() => { dialogs[currentDialogIndex].gameObject.SetActive(true); });
    ...
    public void OpenTable()
    {
        ...
        seq.Append(props.transform.DOScale(1, animationTime).SetEase(Ease.OutBack));
        seq.AppendCallback(() =>
        {
            bomb1.transform.localScale = Vector3.zero;
            bomb1.gameObject.SetActive(true);
            bomb1.transform.DOScale(1, animationTime).SetEase(Ease.OutBack);
        });
        seq.Append(camera.transform.DORotate(Quaternion.AngleAxis(30, camera.transform.right).eulerAngles,
            animationTime).SetEase(Ease.Linear));
    }
    ...
    private void FoldTable()
    {
        ...
        seq.Append(bomb1.transform.DOScale(0, animationTime * 0.5f).SetEase(Ease.InBack));
        seq.Append(bomb2.transform.DOScale(0, animationTime * 0.5f).SetEase(Ease.InBack));
        seq.AppendCallback(() =>
        {
        ...
        seq.Append(props.transform.DOScaleX(0, animationTime).SetEase(Ease.InBack));
        seq.AppendCallback(() => { props.gameObject.SetActive(false); });
        seq.AppendInterval(1f);
        seq.AppendCallback(() => { SceneChanger.Instance.LoadSetupScene(); });
        // seq.Restart();
    }
    ...
    private void SpawnBomb2()
    {
        ...
        bomb2.transform.DOScale(1, animationTime).SetEase(Ease.OutBack);
    }
}